<template>
    <div class="row levelbutton d-sm-none d-md-block d-none d-sm-block" v-if="GameStatus == 'Progressing'">
        <div class="d-grid gap-2 d-flex justify-content-center mb-3 levebar">
            <button type="button" class="btn btn-primary flex-fill text-nowrap" disabled>
                關卡
            </button>
            <div v-for="(i, key) in questions" :key="key" class="grid-item flex-fill d-flex justify-content-between">
                <button type="button" class="btn btn-success flex-fill" :class="{ active: Nowlevel == key + 1 }"
...
export default {
    ...
    computed: {
        ...mapWritableState(gameStore, ['transitionName','GameStatus','Nowlevel']),
