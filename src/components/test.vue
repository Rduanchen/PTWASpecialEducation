    ...
    <div>
      ...
      <ul>
        <li v-for="(voice, index) in voices" :key="index">
          {{ index + 1 }}: {{ voice.name }} ({{ voice.lang }}) - {{ voice.default ? 'Default' : '' }}
        </li>
      </ul>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      speak() {
        ...
        synth.cancel();
        synth.lang = 'zh-TW';
        let utterThis = new SpeechSynthesisUtterance('東漢末年，漢廷因黃巾之亂、北宮伯玉之亂、黑山軍起義、王芬謀廢靈帝、張舉張純叛亂、外戚宦官火拼等一系列事件而動盪不安。184年漢靈帝時期，以張角三兄弟為首爆發黃巾之亂');
