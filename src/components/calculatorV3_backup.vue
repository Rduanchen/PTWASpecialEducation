      ...
      <div class="card CaluculatorBody">
            ...
            <div class="Carrys" v-for="(items, index) in Carry">
              ...
              >
                <template #item="{ element }">
                  <button
                    type="button"
                    class="btn btn-primary m-1 my-btn"
                    :class="{ ButtonLine: this.Carryline[index][0] }"
                    @click="CarryTurnToLine(index)"
                  >
                    {{ element }}
                  </button>
                </template>
              ...
              >
                <template #item="{ element }">
                  <button
                    type="button"
                    class="btn btn-primary m-1 my-btn"
                    :class="{ ButtonLine: this.Carry2line[index][0] }"
                    @click="Carry2TurnToLine(index)"
                  >
                    {{ element }}
                  </button>
                </template>
              ...
              <div class="NumbersContainer">
                <div class="Number" v-for="(item, index2) in items">
                  ...
                  <draggable
                    ...
                    group="Number"
                    item-key="name"
                    class="NumberConainer"
                    @change="GetIndex(index)"
                    @add="NumberCheckInput"
                  >
                    <template #item="{ element }">
                      <button
                        type="button"
                        class="btn btn-primary m-1 my-btn"
                        :class="{
                          ButtonLine: this.ButtonLine[index][index2][0],
                        }"
                        @click="TurnToLine(index, index2)"
                      >
                        {{ element }}
                      </button>
                    </template>
        ...
        <div class="buttons">
          <button
            ...
            v-if="this.GameConfig.FunctionPermission.AddRow"
          >
            新增一行
          </button>
          <button
            @click="removerow"
            class="btn btn-primary m-1"
            v-if="this.GameConfig.FunctionPermission.RemoveRow"
          >
            移除最後一行
          </button>
          <button @click="clear" class="btn btn-primary m-1">
            清除所有數字
          </button>
          <button @click="CheckAnswer" class="btn btn-primary m-1">
            確認答案
          </button>
        </div>
      </div>
      <div class="card Selection">
        <div class="row">
          ...
          <div
            class="setting"
            v-if="
              this.GameConfig.FunctionPermission.CorrectUseUnit == true ||
              this.GameConfig.FunctionPermission.CorrectUnit == true
            "
          >
            <h2>設定</h2>
            <hr />
            ...
            <div v-if="this.GameConfig.FunctionPermission.CorrectUnit">
              ...
              <div class="UnitNumber d-flex">
                ...
                <button
                  type="button"
                  class="btn btn-primary m-1 w-50"
                  @click="SetUnit(1)"
                >
                  +
                </button>
...
export default {
  ...
  methods: {
    SetUnit: function (num) {
      if (
        !(this.FakeData.Unit == 8 && num == 1) &&
        !(this.FakeData.Unit == 2 && num == -1)
      ) {
    ...
    AddRow: function () {
      ...
      this.Num_list.push(temp);
      if (
        this.Num_list.length * (10 ** this.FakeData.Unit - 1) >
        10 ** this.Ans.length
      ) {
        this.Ans.push([]);
        this.Carry.push([]);
    ...
    CheckAnswer: function () {
      ...
      if (this.GameData.Verification.Number) {
        for (var i = 0; i < this.Num_list.length; i++) {
          ...
          console.log(this.GameData.Answer.Number_list[i]);
          if (
            this.GameData.Answer.Number_list[i] != undefined &&
            this.temp != ""
          ) {
            ...
            Gamecheck = false;
            console.warn(
              "Calculation Game: Answer No Found, Please Check the Answer List is Correct"
            );
          }
        }
